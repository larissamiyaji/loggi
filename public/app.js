...
// consumirApi(endpoint, Appquery).then((response) => {
//   const filter = response.data.closestDrivers.drivers.filter(
//     (element) => element.busy === false
//   );
//   filter.map((item) => {
//     console.log(item.busy, item.lat, item.lng);
//   });
// });
// ----------------------------------------------------------
function initMap() {
  ...
  const coord = response.data.closestDrivers.drivers.map(pontodecalor => {
      return new google.maps.LatLng(pontodecalor.lat, pontodecalor.lng)
    })
